...
if False:
    import json
    prices = priceDb.getPullPricesByCode(codes[0],1, 400)
    prices = [{"trade_date": str(p['trade_date']), "close":p['close']} for p in prices]
...
minBalance =balance 
for p1,p2 in iterReduce(iter(prices)):
...
print("total end:", total(prices[-1]['close']), ",trade_num:", trade_num, ",minBalance:", minBalance)
